...
function handeny_setup() {
	/*
		...
		* If you're building a theme based on handeny, use a find and replace
		* to change 'handeny' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'handeny', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'handeny' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'handeny_custom_background_args',
			array(
 ...
 */
function handeny_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'handeny' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'handeny' ),
...
add_action( 'widgets_init', 'handeny_widgets_init' );
...
function handeny_scripts() {
	...
	wp_style_add_data( 'handeny-style', 'rtl', 'replace' );
